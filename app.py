...
import numpy as np
import base64  # Import base64 for CSV download
...
# Define `results` as an empty DataFrame as a fallback
...
if not results.empty:
    results["Review Count"] = pd.to_numeric(results["Review Count"], errors="coerce")  # Convert Review Count to numeric
    sort_by = st.selectbox("Sort results by", ["Distance (km)", "Rating", "Review Count"])
    if sort_by in results.columns:
        results.sort_values(by=sort_by, ascending=(sort_by != "Rating"), inplace=True)
...
def generate_share_link():
    ...
    try:
        ...
        return base_url  # Return base URL if variables are undefined
